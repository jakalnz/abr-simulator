...
`patient = {name, adult, ageMonths, noisy, ears:[{ac[4], bc[4], path 0|1|2, sev 0-3, cm 0-3, latI, latIII, latV}]}`
(thresholds dB HL at 0.5/1/2/4 kHz; `path` 1 = retrocochlear, 2 = neuropathy/ANSD; latencies are optional manual
overrides at 80 dB nHL, 17.1/s). The codec packs this into ~65–90 chars (Word hyperlinks break above 255).
Bump `VERSION` in `codec.js` if the packed layout changes and keep decoding old versions.
